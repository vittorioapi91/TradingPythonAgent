...
def main():
    ...
    try:
        ...
        if args.list_dataflows:
            ...
            for f in flows[:20]:
                print(f"  {f.get('dataflow_id')}: {f.get('name')}")
            if len(flows) > 20:
                print("  ... (truncated)")
            return 0

        if args.generate_db:
            print("Generating PostgreSQL database with all BIS dataflows...")
            downloader.generate_db()
            print("Database generated for BIS dataflows")
            print(f"  Database: bis")
            print("Use --from-db to download datasets using this database")
            return 0
        ...
        if args.from_db and args.dataflow:
            ...
            if data:
                print(f"Download complete: {args.dataflow}")
                print(f"Data saved to PostgreSQL database")
            return 0

        print("Nothing to do. Use --list-dataflows, --generate-db, or --from-db with --dataflow.")
        return 0
